...
R is an open source, **scripting** language. RStudio is a [GUI](https://www.computerhope.com/jargon/g/gui.htm) that is used to interact with R. You need to have R installed in order for RStudio to work. 
  ...
  When you first open RStudio, you should see your screen divided into four quadrants. We will discuss each of these, but for now it is enough to know that they display different information and are helpful to manage your work.  
...
You may need to check that you're working where the code is saved on your computer. If this is the case, go to Session>Set Working Directory>Choose Directory and pick the correct folder. 
...
## Links
RMarkdown files can also handle links to websites and emails. For a website, include the page name in `[]` and the website address in `()`.    

Email is similar, but you need to include `mailto:` in the parentheses. For example, to email Joel, you'd write: [Joel Ledford](mailto:jmledford@ucdavis.edu).    
...
**3. `Objects` in R are a way in which we can store data or operations. We will use these a lot going forward and explain more next we meet. For now, make a new object `pi` as 3.14159265359 by running the following code chunk. You should now see the object `pi` in the environment window in the top right.**  
```{r}
pi <- 3.14159265359
```
...
**5. Push your final code to GitHub!**
Please be sure that you check the `keep md` file in the knit preferences. Check and see that it has uploaded.
