...
**6. In order to use ggplot yourself, you'll need to install and load a package called "ggplot2". Run the following code chunk to do this. (If you already have ggplot2 installed, you will get an error message, and you can click "cancel.")**
```{r}
install.packages("ggplot2")
library(ggplot2)
```

**7. To make a plot, we need to first specify the data and map the aesthetics. The aesthetics include how each variable in our data set will be used. In the example below, we will use the aes() function to identify the x and y variables in the plot. We will use the "point" geom to create a scatter plot.  Run the code block below for an initial plot!
...
names(iris)
```
Note that there were three commands run, each of which gave us a way to figure out potential variables to use! Can you find the variable names via all three methods?
