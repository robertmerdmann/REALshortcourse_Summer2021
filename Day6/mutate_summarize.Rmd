...
```{r, eval=FALSE}
edu_dat %>%
  select(mc2_pre, mc2_post, Course, admit_level) %>%
  mutate(mc2_pre_clean = ifelse(mc2_pre = "-99", NA, mc2_pre),
         mc2_post_clean = ifelse(mc2_post = "-99", NA, mc2_post)) %>%
  mutate(mc2_pre_tf = (mc2_pre_clean == "A"),
         mc2_post_tf = (mc2_post_clean == "A"))
